...
def calculate_pi(partitions, output_uri):
    ...
    with SparkSession.builder.appName("Calculate Pi with PySpark").getOrCreate() as spark:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate Pi using PySpark")
    parser.add_argument(
        '--partitions', default=2, type=int,
        help="The number of parallel partitions to use when calculating pi.")
    parser.add_argument(
        '--output_uri', required=True,
        help="The URI where output is saved, typically a GCS bucket URI, e.g., gs://bigdata_pyspark/pi-output.")
